...
function VerifyEmail() {
  ...
  document.title = 'LIneA Course | Verify E-mail';
  ...
  useEffect(() => {
    ...
    getVerifyEmail({ uid, token }).then((res) => {
      if (res.error) {
        setObjectMensage({
          type: 'error',
          message: res.error,
        ...
        <div className={classes.verifyEmail}>
          <Typography
            ...
            className={`${classes.title} ${
              objectMensage.type === 'success' ? classes.success : classes.error
            }`}
          >
            {objectMensage.type}
          </Typography>
